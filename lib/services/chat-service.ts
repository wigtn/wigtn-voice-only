...
export function matchPlaceFromUserMessage(
  ...
  const numMatch = trimmed.match(
    /^(?:(\d+)\s*번|첫\s*번째|두\s*번째|세\s*번째|네\s*번째|다섯\s*번째|(\d+))$/
  );
  const numMap: Record<string, number> = { 첫: 1, 두: 2, 세: 3, 네: 4, 다섯: 5 };
  let index = -1;

  if (numMatch) {
    if (numMatch[1]) index = parseInt(numMatch[1], 10) - 1;
    else if (numMatch[2]) index = parseInt(numMatch[2], 10) - 1;
  } else {
    const firstWord = trimmed.split(/\s/)[0] || '';
    if (/^[一二三四五]$/.test(firstWord) || /^[1-5]$/.test(firstWord)) {
      index =
        firstWord === '一' || firstWord === '1'
          ? 0
          : firstWord === '二' || firstWord === '2'
            ? 1
            : firstWord === '三' || firstWord === '3'
              ? 2
              : firstWord === '四' || firstWord === '4'
                ? 3
                : 4;
...
export function extractDataFromMessage(
  ...
  // 전화번호 패턴
  const phoneMatch = m.match(/(0\d{1,2}-?\d{3,4}-?\d{4})|(010\d{8})/);
  if (phoneMatch) {
    const raw = (phoneMatch[1] || phoneMatch[2] || '').replace(/-/g, '');
